...
sll part1(strings_t input)
{
    ...
    for (size_t ii = 0; ii < num_rows; ++ii)
    {
        ...
        for (size_t jj = 0; jj < num_cols; ++jj)
        {
            ...
            bool is_number = is_a_number(cell);
            if (is_number)
            {
            ...
            bool this_is_the_first_non_number_cell_after_a_number = !is_number && start_col_opt.has_value() && end_col == jj - 1;
            bool this_is_the_last_column_and_this_cell_is_a_number = is_number && jj == num_cols - 1;
            if (this_is_the_first_non_number_cell_after_a_number || this_is_the_last_column_and_this_cell_is_a_number)
            {
                size_t start_col = start_col_opt.value();
                int the_number = std::stoi(input[ii].substr(start_col, jj - start_col + 1));
                if (isnt_a_number_nor_period(cell))
                {
...
int main ()
{
    ...
    if (aoc::results(results_test_1, expected_test_result_1))
    {
        ...
        // I was missing numbers along the right column. Fixed. New guess: 541515. Wrong. No longer gives hints
        // Mmm. I was handling those final columns incorrectly, adding all. New guess: 540131
    }

    std::cout << "\nPart 2\n\n";
    sll results_test_2 = part2(test_data);
    sll expected_test_result_2 = 467835;
